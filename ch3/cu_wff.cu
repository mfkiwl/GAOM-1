...
float warpReduceSum(float val) {//warp内求和
...
inline cufftComplex ComplexScale(cufftComplex a, float s)//复矩阵乘参数s，用于实现归一化
{
	cufftComplex c;
	c.x = s*a.x;
	c.y = s*a.y;
...
static __device__ __host__
inline cufftComplex ComplexMul(cufftComplex a, cufftComplex b)//两复数矩阵相乘
{
	cufftComplex c;
	c.x = a.x * b.x - a.y * b.y;
	c.y = a.x * b.y + a.y * b.x;
...
[yf xf]=meshgrid(-fix(nn/2):nn-fix(nn/2)-1,-fix(mm/2):mm-fix(mm/2)-1); mm, nn are padded height&width fix向0方向取整
NOTE:
...
void gen_xf_yf_Kernel(cufftReal *d_out_xf,
	cufftReal *d_out_yf,
	int iWidth, int iHeight)//将坐标移动至图像中心
{
...
iSize: size of the matrices 实际上为整个矩阵模的大小
OUTPUT:

	...
	Timer& m_Time,int m_iFFTSubW,int m_iFFTSubH)
{
	...
	cudaMemset(m_d_iU, 0, sizeof(int)*m_iNumSize);//初始化为0
...
//	double timeall = (time5 - time0)/getTickFrequency();
	...
	cudaEventElapsedTime(&t_hessian, fft_cc, hessian );
...
void icgn_prepare(ICGN_d_Handle &m_Handle, int ***&m_dPXY, float ***&m_dP, int ** &m_iIterationNum)
{
	...
	int m_iNumSize = m_iNumberX*m_iNumberY;//the number of points of interest(POIs)
	int m_iSubwindowSize = m_iSubsetW*m_iSubsetH;//the size of subset

	//Initialize the parameters for IC-GN algorithm
	cucreateptr(m_iIterationNum, m_iNumberY, m_iNumberX); // iteration step taken at each POI / m_iIterationNum 定位每一个POI的迭代次数
	...
	Timer &m_Time)
{
	...
	m_Handle.m_iSubsetX = m_iSubsetX;//m_iSubsetX=16
	m_Handle.m_iSubsetY = m_iSubsetY;
	m_Handle.m_iMarginX = m_iMarginX;//m_iMarginX=5，POI距离最近的中心点边界
	m_Handle.m_iMarginY = m_iMarginY;
	m_Handle.m_iGridSpaceX = m_iGridSpaceX;// m_iGridSpaceX=5
	...
	//Estimate the number of points of interest(POIs)
	m_Handle.m_iNumberX = int(floor((m_Handle.m_iWidth - m_iSubsetX * 2 - m_iMarginX * 2) / float(m_iGridSpaceX))) + 1;//floor向下取整，不大于x的最大的整数
